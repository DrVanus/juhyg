...
class PriceViewModel: ObservableObject {
    ...
    private func coingeckoID(for symbol: String) -> String {
        // Normalize symbol by removing any "usdt" suffix, then map to CoinGecko ID
    ...
    func startLiveUpdates(coinID: String) {
        ...
        liveData.removeAll()
        // Start CoinGecko polling-based live price updates
        LivePriceManager.shared.connect(symbol: symbol)
        livePriceCancellable?.cancel()
        livePriceCancellable = LivePriceManager.shared.pricePublisher
            .sink { [weak self] dict in
                guard let self = self else { return }
                let id = self.coingeckoID(for: self.symbol)
                DispatchQueue.main.async {
                    self.price = dict[id] ?? self.price
                }
            }
