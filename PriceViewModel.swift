...
class PriceViewModel: ObservableObject {
    ...
    func startLiveUpdates(coinID: String) {
        ...
        // Subscribe to real-time publisher
        livePriceCancellable = wsService
            .pricePublisher(for: [coinID], interval: 5.0)
            .sink { [weak self] priceDict in
                guard let self = self,
                      let newPrice = priceDict[coinID] else { return }
                Task { @MainActor in
                    withAnimation(.linear(duration: 0.2)) {
                        self.price = newPrice
                    }
